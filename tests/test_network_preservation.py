...
class TestNetworkPreservation:
    ...
    def migration_results(self):
        ...
        success = migrator.migrate(
            golden_old_path=str(project_root / "rcnltxekvzwcslf-y-or-x-004-occndbtier_25.1.102.yaml"),
            template_old_path=str(project_root / "occndbtier_custom_values_25.1.102.yaml"),
            template_new_path=str(project_root / "occndbtier_custom_values_25.1.200.yaml"),
        ...
        # Load results
        with open(project_root / 'test_network_output.yaml', 'r') as f:
            result_config = yaml.safe_load(f)

        with open(project_root / 'test_network_log.json', 'r') as f:
        ...
        # Cleanup
        for file_path in [project_root / "test_network_output.yaml", project_root / "test_network_log.json"]:
            if file_path.exists():
                file_path.unlink()
    ...
    def test_istio_sidecar_injection_preserved(self, migration_results):
        ...
        for path in critical_paths:
            ...
            if annotations:
                if isinstance(annotations, list):
                    annotations_dict = self.convert_array_annotations_to_dict(annotations)
        ...
        assert injection_found, "Istio sidecar injection should be preserved in at least one component"
                ...
                for proxy_annotation in proxy_annotations:
                    if proxy_annotation in annotations_dict:
                        ...
                        if "CPU" in proxy_annotation:
                            assert value == "2", f"CPU annotation should be '2', got '{value}'"
                        elif "Memory" in proxy_annotation:
                            assert value == "2Gi", f"Memory annotation should be '2Gi', got '{value}'"

        assert found_proxy_configs >= 4, f"Should find at least 4 proxy resource configs, found {found_proxy_configs}"
        ...
        for path in critical_paths:
            ...
            if annotations:
                ...
                if "proxy.istio.io/config" in annotations_dict:
                    assert annotations_dict["proxy.istio.io/config"] == "{concurrency: 2}"
                    config_found = True

        assert config_found, "Istio proxy config should be preserved in at least one component"
    ...
    def test_f5_load_balancer_labels_preserved(self, migration_results):
        ...
        sqlgeorepsvclabels = self.get_nested_value(config, 'api.externalService.sqlgeorepsvclabels')

        assert sqlgeorepsvclabels is not None, "sqlgeorepsvclabels should exist"
        assert isinstance(sqlgeorepsvclabels, list), "sqlgeorepsvclabels should be a list"
        ...
        for service_config in sqlgeorepsvclabels:
            if 'labels' in service_config and isinstance(service_config['labels'], list):
                labels_dict = self.convert_array_annotations_to_dict(service_config['labels'])
        ...
        assert f5_labels_found >= 3, f"Should find at least 3 F5 labels, found {f5_labels_found}"
    ...
    def test_oracle_cnf_annotations_preserved(self, migration_results):
        ...
        dbreplsvc = self.get_nested_value(config, 'db-replication-svc.dbreplsvcdeployments')
        ...
        if dbreplsvc and isinstance(dbreplsvc, list):
            ...
            assert oracle_cnf_found, "Oracle CNF annotation should be preserved in db-replication-svc"
    ...
    def test_metallb_annotations_preserved(self, migration_results):
        ...
        if dbreplsvc and isinstance(dbreplsvc, list):
            metallb_found = False
            for deployment in dbreplsvc:
                service_annotations = deployment.get('service', {}).get('annotations', {})
                if 'metallb.universe.tf/address-pool' in service_annotations:
                    assert service_annotations['metallb.universe.tf/address-pool'] == "oam"
                    metallb_found = True

            assert metallb_found, "MetalLB annotation should be preserved in db-replication-svc"
    ...
    def test_network_categories_preserved(self, migration_results):
        ...
        for category in expected_categories:
            assert category.replace('_', '') in target_value.replace('_', ''), f"Category {category} should be preserved"
    ...
    def test_migration_statistics(self, migration_results):
        ...
        assert statistics['by_action_type']['OVERWRITE'] >= 50, "Should have substantial number of overwrites"

        # Verify we have network preservation activity
        assert summary['total_entries'] >= 100, "Should have processed substantial number of entries"

        # Verify some manual review items (expected for deleted/changed paths)
        assert summary['manual_review_required'] > 0, "Should have some items requiring manual review"
            ...
            if data and isinstance(data, list):
                for item in data:
                    ...
                    # Check direct labels
                        ...
                        labels_dict = self.convert_array_annotations_to_dict(item['labels'])
                        if 'app' in labels_dict:
                            assert labels_dict['app'] == "occne_infra"
                            app_labels_found += 1

        assert app_labels_found >= 2, f"Should find at least 2 service app labels, found {app_labels_found}"